...
describe('Codex CLI installer config repair', () => {
  ...
  it('updates existing plugin enablement in place', () => {
    const input = [
      '[plugins."copilot-mem@ramikhafagi96"]',
      'enabled = true',
      '',
    ...
    const result = setTomlPluginEnabled(input, 'copilot-mem@ramikhafagi96', false);

    expect(result).toContain('[plugins."copilot-mem@ramikhafagi96"]\nenabled = false');
